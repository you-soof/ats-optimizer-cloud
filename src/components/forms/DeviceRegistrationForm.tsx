...
const deviceSchema = z.object({
  ...
  latitude: z.number().min(59).max(71, 'Latitude must be in Finland range (59-71)'),
  longitude: z.number().min(19).max(32, 'Longitude must be in Finland range (19-32)'),
  insulation_level: z.enum(['poor', 'average', 'good', 'excellent']),
  floor_area: z.number().min(1).max(500, 'Floor area must be 1-500 m²'),
  volume: z.number().min(1).max(1500, 'Volume must be 1-1500 m³'),
  heat_pump_type: z.enum(['air_source', 'ground_source', 'water_source', 'hybrid']),
...
export function DeviceRegistrationForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="glass-card p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            />
            {errors.device_id && (
              <p className="text-xs text-destructive">{errors.device_id.message}</p>
            )}
          </div>
          <div className="space-y-2">
      ...
      {/* Location */}
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Location (Finland)</h2>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
      ...
      {/* Building Properties */}
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Building Properties</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <Label>Insulation Level</Label>
            <Select onValueChange={(value) => setValue('insulation_level', value as InsulationLevel)}>
            ...
            {errors.insulation_level && (
              <p className="text-xs text-destructive">{errors.insulation_level.message}</p>
            )}
          </div>
          <div className="space-y-2">
            ...
            />
            {errors.floor_area && (
              <p className="text-xs text-destructive">{errors.floor_area.message}</p>
            )}
          </div>
          <div className="space-y-2">
      ...
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Heat Pump Configuration</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <Label>Heat Pump Type</Label>
            <Select onValueChange={(value) => setValue('heat_pump_type', value as HeatPumpType)}>
              <SelectTrigger className="bg-secondary border-border">
                <SelectValue placeholder="Select type" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="air_source">Air Source</SelectItem>
                <SelectItem value="ground_source">Ground Source</SelectItem>
                <SelectItem value="water_source">Water Source</SelectItem>
                <SelectItem value="hybrid">Hybrid</SelectItem>
              </SelectContent>
            </Select>
            {errors.heat_pump_type && (
              <p className="text-xs text-destructive">{errors.heat_pump_type.message}</p>
            )}
          </div>
          <div className="space-y-2">
            ...
            />
            {errors.rated_power && (
              <p className="text-xs text-destructive">{errors.rated_power.message}</p>
            )}
          </div>
          <div className="space-y-2">
            ...
            />
            {errors.cop_rating && (
              <p className="text-xs text-destructive">{errors.cop_rating.message}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Comfort Settings */}
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Comfort Settings</h2>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            {errors.comfort_min_temp && (
              <p className="text-xs text-destructive">{errors.comfort_min_temp.message}</p>
            )}
          </div>
          <div className="space-y-2">
            ...
            {errors.comfort_max_temp && (
              <p className="text-xs text-destructive">{errors.comfort_max_temp.message}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-foreground">Virtual Power Plant</h2>
